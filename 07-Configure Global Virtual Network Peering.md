...
You will be using an Azure resource group named RG1lod28508511 that contains an existing storage account named sa28508511 for use with Cloud Shell, two virtual networks named VNET (East US)  and VNET2 (East US 2), both with Address space 10.1.0.0/16 and  subnet 1 10.1.0.0./24, and two virtual machines named VM1 (East US) and VM2 (East US 2).

# Review the created Azure resources

Sign in to the Azure portal as Admin-28508511@cloudslice.onmicrosoft.com using pB5rSd!G9* as the password.
...
- On the Azure portal home page, select Resource groups, select RG1lod28508511, and then select VM2-nic.
...
- Configure an Azure Cloud Shell **Bash** session by using the existing **RG1lod28508511** resource group, an existing storage account named **sa28508511** in the **East US** region, and a new file share named cloud-shell-share.
  ...
  - In Resource group, ensure that **Use existing** is selected, and then ensure that **RG1lod28508511** is selected.

  - In Storage account, ensure that **Use Existing** is selected, and then ensure that **sa28508511** is selected.
